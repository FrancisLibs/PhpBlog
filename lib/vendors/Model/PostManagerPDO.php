...
class PostManagerPDO extends PostManager
{
  ...
  public function getList($debut = -1, $limite = -1)
  {
    ...
    $sql = 'SELECT posts.id, title, chapo, contenu, edition_date, update_date, user_id, users.login AS author_name FROM posts INNER JOIN users ON posts.user_id = users.id ORDER BY posts.id DESC';
  ...
  {
    $requete = $this->dao->prepare('SELECT posts.id, title, chapo, contenu, edition_date, update_date, user_id, users.login AS author_name FROM posts INNER JOIN users ON posts.user_id = users.id WHERE posts.id = :id');
